...
@magicgui(call_button='run segmentation',  
          layout='vertical',
          label_head = dict(widget_type='Label', label=f'<h2> cellpose </h2>'), 
...
def widget(#label_logo, 
    ...
    if clear_previous_segmentations:
        for seg in widget.cellpose_layers:
            for layer in seg:
    ...
    def _new_layers(masks, flows_orig):
        ...
        if widget.output_flows.value:
            layers.append(viewer.add_image(flows, name=image_layer.name + '_flows' + widget.iseg, visible=False, rgb=True))
            layers.append(viewer.add_image(cellprob, name=image_layer.name + '_cellprob' + widget.iseg, visible=False))
        if widget.output_outlines.value:
            layers.append(viewer.add_labels(outlines, name=image_layer.name + '_outlines' + widget.iseg, visible=False))
        layers.append(viewer.add_labels(masks, name=image_layer.name + '_masks' + widget.iseg, visible=False))
    ...
    def _new_segmentation(segmentation):
        ...
        try:
            ...
            if len(widget.cellpose_layers) > 1:
                for seg in widget.cellpose_layers:
                    for layer in seg:
                        layer.visible = False
            widget.cellpose_layers[-1][-1].visible = True
...
def _compute_diameter(event):
    if widget.model_type.value == 'custom':
        ...
        napari.utils.notifications.show_info('some message')
